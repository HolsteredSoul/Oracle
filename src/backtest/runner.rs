...
pub struct BacktestReport {
    ...
    pub balance_history: Vec<(DateTime<Utc>, f64)>,
    /// Per-trade log.
...
impl Backtester {
    ...
    pub fn run(&self, markets: &[ResolvedMarket], initial_bankroll: f64) -> BacktestReport {
        let mut state = AgentState::new(initial_bankroll);
        let mut risk_manager = RiskManager::new(self.risk_config.clone());
        ...
        for market in markets {
            ...
            if abs_edge < threshold {
                ...
                let outcome = if market.resolved_yes { 1.0 } else { 0.0 };
                brier_sum += (market.estimated_probability - outcome).powi(2);
                continue;
            }
            ...
            let win_prob = match &side {
                ...
                Side::No => 1.0 - market.estimated_probability,
            };
            let market_price = match &side {
                Side::Yes => market.market_price_yes,
                Side::No => 1.0 - market.market_price_yes,
            };
            let payout_ratio = if market_price > 0.0 && market_price < 1.0 {
                (1.0 - market_price) / market_price
            } else {
                continue;
            };
            ...
            let bet_amount = (bet_frac * state.bankroll).max(0.0);
            ...
            let trade_return = pnl / bet_amount.max(0.01);
            ...
            let dd = if peak > 0.0 { 1.0 - state.bankroll / peak } else { 0.0 };
            if dd > max_dd {
                max_dd = dd;
            }
        ...
        let return_pct = if initial_bankroll > 0.0 {
            (state.bankroll - initial_bankroll) / initial_bankroll * 100.0
        } else {
            0.0
        };
...
mod tests {
    ...
    fn test_profitable_backtest() {
        ...
        let report = bt.run(&markets, 100.0);
        assert!(report.final_bankroll > 100.0, "Should be profitable: ${:.2}", report.final_bankroll);
    ...
    fn test_losing_backtest() {
        ...
        let report = bt.run(&markets, 100.0);
        assert!(report.final_bankroll < 100.0, "Should lose money: ${:.2}", report.final_bankroll);
    ...
    fn test_empty_markets() {
        let bt = default_backtester();
        let report = bt.run(&[], 100.0);
        assert_eq!(report.total_trades, 0);
        assert!((report.final_bankroll - 100.0).abs() < 1e-10);
    ...
    fn test_agent_dies_on_bankrupt() {
        let bt = Backtester::new(
            EdgeConfig { min_edge: 0.01, weather_threshold: 0.02, ..EdgeConfig::default() },
            KellyConfig { multiplier: 1.0, max_bet_pct: 0.95, min_bet_size: 0.1, commission_per_trade: 0.0 },
            RiskConfig { max_exposure_pct: 1.0, ..RiskConfig::default() },
        );
        ...
        let report = bt.run(&markets, 10.0);
        assert!(report.final_bankroll <= 0.0 || report.total_trades < 20);
    }
